...
Last updated: 18/09/2015 5:29PM 1st commit
...
q1 = ax1.bar(ind,atl_EPRbnds,width,color='k')
q2 = ax1.bar(ind+width,atl_evapbnds,width,color='red')
q3 = ax1.bar(ind+2*width,atl_precbnds,width,color='blue')
q4 = ax1.bar(ind+3*width,atl_run5deg,width,color='green')
...
ax1.set_ylabel('Freshwater fluxes (Sv)',fontsize=16)
ax1.set_title('Atlantic Ocean')

ax2 = pl.subplot(3,1,2)
w1 = ax2.bar(ind,pac_EPRbnds,width,color='k')
w2 = ax2.bar(ind+width,pac_evapbnds,width,color='red')
w3 = ax2.bar(ind+2*width,pac_precbnds,width,color='blue')
w4 = ax2.bar(ind+3*width,pac_run5deg,width,color='green')
pl.axhline(y=0,color='lightgrey',ls='--',linewidth=0.5) # add line to show y=0
pl.xticks(pl.linspace(3*width/2,19+3*width/2,20))
ax2.xaxis.set_major_formatter(pl.NullFormatter())
pl.ylim(-0.55,1.1)
pl.yticks(pl.linspace(-0.5,1,4))
ax2.set_ylabel('Freshwater fluxes (Sv)',fontsize=16)
ax2.set_title('Pacific Ocean')

ax3 = pl.subplot(3,1,3)
s1 = ax3.bar(ind,ind_EPRbnds,width,color='k')
s2 = ax3.bar(ind+width,ind_evapbnds,width,color='red')
s3 = ax3.bar(ind+2*width,ind_precbnds,width,color='blue')
s4 = ax3.bar(ind+3*width,ind_run5deg,width,color='green')
pl.axhline(y=0,color='lightgrey',ls='--',linewidth=0.5) # add line to show y=0
pl.xticks(pl.linspace(3*width/2,19+3*width/2,20))
ax3.set_xticklabels(bnd_lbls,rotation=45)
pl.ylim(-0.55,1.1)
pl.yticks(pl.linspace(-0.5,1,4))
ax3.set_ylabel('Freshwater fluxes (Sv)',fontsize=16)
ax3.set_title('Indian Ocean')
ax3.legend((s1[0],s2[0],s3[0],s4[0]),('$E-P-R$','$E$','$P$','$R$'),loc=2)

fig.suptitle('Ocean freshwater fluxes in 5$^\circ$ latitude bands',fontsize=17,
             x=0.55,y=0.95)
