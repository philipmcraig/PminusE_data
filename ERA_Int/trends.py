# -*- coding: utf-8 -*-
"""
Created on Tue Nov 24 11:31:56 2015
...
oc_evap = evap_tot*(1-lsm)
oc_prec = prec_tot*(1-lsm)
...
oc_evapint = pl.zeros([oc_evap.shape[0],oc_evap.shape[1]])
oc_precint = pl.zeros_like(oc_evapint)
atl_evapint = pl.zeros_like(oc_evapint)
atl_precint = pl.zeros_like(oc_evapint)
pac_evapint = pl.zeros_like(oc_evapint)
pac_precint = pl.zeros_like(oc_evapint)
...
for yr in range(len(years)):
    for mt in range(0,12):
        oc_evapint[yr,mt] = NetEmPCalc(areas,oc_evap[yr,mt]*factor,rho)
        oc_precint[yr,mt] = NetEmPCalc(areas,oc_prec[yr,mt]*factor,rho)
...
oc_precscl = pl.zeros_like(oc_evapscl)
atl_evapscl = pl.zeros_like(oc_evapscl)
atl_precscl = pl.zeros_like(oc_evapscl)

for yr in range(len(years)):
    oc_evapscl[yr] = FluxScaled(areas,atlmask,oc_evapint[yr])*scl_fctr
    oc_precscl[yr] = FluxScaled(areas,atlmask,oc_precint[yr])*scl_fctr
    atl_evapscl[yr] = FluxScaled(areas,atlmask,atl_evapint[yr])*scl_fctr
    atl_precscl[yr] = FluxScaled(areas,atlmask,atl_precint[yr])*scl_fctr
    
f = linregress(years,oc_evapscl)