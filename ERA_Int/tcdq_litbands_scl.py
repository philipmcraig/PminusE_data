...
fluxfile = Dataset('tcdq_mean.nc',r')
tcdq = fluxfile.variables['tcdq'][:] # vertically integrated moisture flux divergence in kg/m^2s
...
# Multiply each tcdq array by each mask to extract correct flux:
atl_tcdq = tcdq*ABmask
pac_tcdq = tcdq*Pmask
ind_tcdq = tcdq*Imask
...
# Atlantic Ocean:
atl_tcdqbnds = NetEmPBands(atl_ind,areas,pl.squeeze(atl_tcdq),rho)
...
#ax3.legend((s1[0],s2[0],s3[0],s4[0]),('$E-P-R$','$E$','$P$','$R$'),loc=0)
