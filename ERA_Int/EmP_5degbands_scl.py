...
Last updated: 18/09/2015 5:26PM 1st commit
...
atl_EmPscl = FluxBandScaled(areas,ABmask,bnd_ind,atl_EmPbnds)*factor
...
pac_EmPscl = FluxBandScaled(areas,Pmask,bnd_ind,pac_EmPbnds)*factor
...
ind_EmPscl = FluxBandScaled(areas,Imask,bnd_ind,ind_EmPbnds)*factor
...
q1 = ax1.bar(ind,atl_EmPscl,width,color='k')
...
w1 = ax2.bar(ind,pac_EmPscl,width,color='k')
...
s1 = ax3.bar(ind,ind_EmPscl,width,color='k')
...
pl.suptitle('Ocean freshwater fluxes in 5$^\circ$ latitude bands scaled by area',
            fontsize=17,y=0.97,x=0.55)
